...
> Implement issue #{N} (part of spec #{M}). Branch `{branch}` off `{base}`; treat the base as given. Use /tdd ($tdd in Codex) with pre-agreed seams where applicable. Subagents are encouraged for exploration and second opinions. All repo gates green when finished, commit, stop with a clean worktree, and wait. Never push or open a PR on your own, and merge or rebase only when the orchestrator explicitly instructs it; the orchestrator owns integration.
